...
(function () {
    ...
    let kešCena = {};
    let kurs = null; // { datum, eur_rsd, usd_rsd, izvor }
    ...
    const fRsd = (n) => (typeof n === 'number' && isFinite(n))
        ? Math.round(n).toLocaleString('en-US')
        : null;

    // Iz USD izračunaj EUR (preko EUR_RSD i USD_RSD)
    ...
    // Formira tekst vrednosti u sve tri valute: "$24.56 / €22.79 / 2,510 RSD"
    function vrednostiUTriValute(usd) {
        if (typeof usd !== 'number' || !isFinite(usd)) return '';
        let tekst = `$${fUsd(usd)}`;
        const eur = usdUEur(usd);
        if (eur !== null) tekst += ` / €${fUsd(eur)}`;
        if (kurs && kurs.usd_rsd) tekst += ` / ${fRsd(usd * kurs.usd_rsd)} RSD`;
        return tekst;
    }
    ...
    function pokreniPracenje(adresa, token) {
        ...
        // Učitaj kurs u pozadini (ne blokira praćenje)
        (async () => {
            try {
